...
export function AdjustEntryPage() {
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <h1 className="text-[20px] font-semibold text-gray-900 dark:text-white">Ajuste de Lançamento</h1>

      {/* Search */}
      <div className="bg-white/70 dark:bg-white/5 backdrop-blur-xl border border-white/60 dark:border-white/10 rounded-2xl shadow-glass dark:shadow-glass-dark p-5">
        ...
        {searchError && <p className="text-[14px] text-red-500 mt-2">{searchError}</p>}
      </div>
      ...
      {loadingRecent ? (
        ...
        <>
          ...
          {operations.length === 0 && wasteEvents.length === 0 ? (
            <p className="text-center text-[15px] text-gray-400 dark:text-gray-500 py-8">Nenhuma operação registrada.</p>
          ) : (
            <div className="space-y-3">
              {operations.map((o) => (
                <div key={o.operation.id} className="bg-white/70 dark:bg-white/5 backdrop-blur-xl border border-white/60 dark:border-white/10 rounded-2xl shadow-glass dark:shadow-glass-dark overflow-hidden animate-fadeIn">
                  <div className="px-5 py-3.5 border-b border-gray-100 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-800/60 flex items-center justify-between">
                    <div>
                      <p className="text-[14px] font-semibold text-gray-900 dark:text-white">
                        Operação #{o.operation.id}
                        <span className="ml-2 font-normal text-gray-500 dark:text-gray-400">·</span>
                        <span className="ml-2 font-normal text-gray-700 dark:text-gray-200">
                    ...
                    <span className={`shrink-0 text-[12px] font-medium px-2 py-0.5 rounded-full ${
                      o.operation.status === 'completed' ? 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400' :
                      o.operation.status === 'contingency' ? 'bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400' :
                    ...
                    {o.entries.map((e) => (
                      <div key={e.id} className="flex items-center justify-between px-5 py-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="text-[14px] text-gray-700 dark:text-gray-200">{entryDisplayName(e)}</span>
                          {(e.user_identifier ?? e.registration_number) &&
                            ...
                            <span className="text-[12px] text-gray-400 dark:text-gray-500">({e.user_identifier ?? e.registration_number})</span>
                          )}
                        </div>
                        <div className="flex items-center gap-3 shrink-0">
                          <span className="text-[16px] font-bold text-gray-900 dark:text-white">{e.sheets} folhas</span>
          ...
          {wasteEvents.length > 0 && (
            <div className="space-y-3">
              <p className="text-[12px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-widest px-1">Erros e folhas em branco</p>
              {wasteEvents.map((w) => (
                <div key={`waste-${w.id}`} className="bg-white/70 dark:bg-white/5 backdrop-blur-xl border border-white/60 dark:border-white/10 rounded-2xl shadow-glass dark:shadow-glass-dark overflow-hidden animate-fadeIn">
                  <div className="px-5 py-3.5 border-b border-gray-100 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-800/60 flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${w.type === 'error' ? 'bg-red-50 dark:bg-red-950/40 text-red-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500'}`}>
                        {w.type === 'error' ? <AlertTriangle size={15} /> : <FileX size={15} />}
                      </div>
                      <div>
                        <p className="text-[14px] font-semibold text-gray-900 dark:text-white">
                          {w.type === 'error' ? 'Erro de impressão' : 'Folhas em branco'}
                          {w.user_name && <span className="ml-2 font-normal text-gray-700 dark:text-gray-200">· {w.user_name}</span>}
                        </p>
                        <p className="text-[12px] text-gray-400 dark:text-gray-500 mt-0.5">
                    ...
                    <span className="shrink-0 text-[14px] font-bold text-gray-700 dark:text-gray-200">{w.sheets} fls</span>
                  </div>
                  ...
                  {w.type === 'error' && (
                    <div className="flex items-center justify-between px-5 py-3">
                      <p className="text-[13px] text-gray-400 dark:text-gray-500">Sem lançamentos — erros de impressão não debitam cota.</p>
                      <Button variant="ghost" size="sm"
      ...
      <Modal open={!!adjustEntry && !confirmAdjust} onClose={() => setAdjustEntry(null)} title="Ajustar Lançamento" size="sm">
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Motivo (obrigatório)</label>
            <textarea
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              rows={3}
              placeholder="Descreva o motivo do ajuste..."
              className="w-full px-3 py-2 text-[15px] border border-gray-200 dark:border-gray-700 rounded-xl outline-none focus:border-gray-400 focus:ring-2 focus:ring-gray-200 resize-none bg-white/70 dark:bg-gray-900/70 dark:text-white dark:placeholder:text-gray-600"
            />
          </div>
        </div>
      ...
      <Modal open={confirmAdjust} onClose={() => setConfirmAdjust(false)} title="Confirmar Ajuste" size="sm">
        <div className="space-y-2 mb-4">
          <div className="flex justify-between text-[14px]">
            <span className="text-gray-500 dark:text-gray-400">Lançamento</span>
            <span className="font-medium dark:text-white">#{adjustEntry?.id}</span>
          </div>
          <div className="flex justify-between text-[14px]">
            <span className="text-gray-500 dark:text-gray-400">Usuário</span>
            <span className="font-medium dark:text-white">{adjustEntry ? entryDisplayName(adjustEntry) : '—'}</span>
          </div>
          <div className="flex justify-between text-[14px]">
            <span className="text-gray-500 dark:text-gray-400">Alteração</span>
            <span className="dark:text-white">
              <span className="line-through text-gray-400 dark:text-gray-500">{adjustEntry?.sheets}</span>
              {' '}&rarr;{' '}
              <strong>{newSheets} folhas</strong>
            </span>
          </div>
          <div className="pt-2 border-t border-gray-100 dark:border-gray-800">
            <p className="text-[13px] text-gray-500 dark:text-gray-400">Motivo:</p>
            <p className="text-[14px] text-gray-700 dark:text-gray-200 italic mt-0.5">"{reason}"</p>
          </div>
        </div>
        {adjustError && <p className="text-[14px] text-red-500 dark:text-red-400 mb-3">{adjustError}</p>}
        <div className="flex gap-2">
      ...
      <Modal open={confirmAdjustWaste} onClose={() => setConfirmAdjustWaste(false)} title="Confirmar Ajuste" size="sm">
        <div className="space-y-2 mb-4">
          <div className="flex justify-between text-[14px]">
            ...
            <span className="font-medium dark:text-white">#{adjustWaste?.id}</span>
          </div>
          <div className="flex justify-between text-[14px]">
            <span className="text-gray-500 dark:text-gray-400">Tipo</span>
            <span className="font-medium dark:text-white">{adjustWaste?.type === 'error' ? 'Erro de impressão' : 'Folhas em branco'}</span>
          </div>
          <div className="flex justify-between text-[14px]">
            <span className="text-gray-500 dark:text-gray-400">Alteração</span>
            <span className="dark:text-white">
              <span className="line-through text-gray-400 dark:text-gray-500">{adjustWaste?.sheets}</span>
              {' '}&rarr;{' '}
          ...
          <div className="pt-2 border-t border-gray-100 dark:border-gray-800">
            <p className="text-[13px] text-gray-500 dark:text-gray-400">Motivo:</p>
            <p className="text-[14px] text-gray-700 dark:text-gray-200 italic mt-0.5">"{wasteReason}"</p>
          </div>
        </div>
        {adjustWasteError && <p className="text-[14px] text-red-500 dark:text-red-400 mb-3">{adjustWasteError}</p>}
        <div className="flex gap-2">
